...
describe('down', () => {
    ...
    beforeEach(() => {
        ...
        migrationsDirLoadMigration = jest.spyOn(migrationsDir, 'loadFilesToBeMigrated').mockResolvedValue(migration);
        jest.spyOn(migrationsDb, 'getDdb').mockResolvedValue(new DynamoDBClient());

        migrationsDbDeleteMigrationFromMigrationsLogDb = jest
            .spyOn(migrationsDb, 'deleteMigrationFromMigrationsLogDb')
    ...
    it('should yield empty list when nothing to downgrade', async () => {
        statusModuleStatus.mockResolvedValue(
            Promise.resolve([{ fileName: '20160609113224-some_migration.js', appliedAt: 'PENDING' }]),
        );
    ...
    it('should yield errors that occurred when deleting from the migrationsLogDb', async () => {
        migrationsDbDeleteMigrationFromMigrationsLogDb.mockReturnValue(Promise.reject(new Error('Could not delete')));
