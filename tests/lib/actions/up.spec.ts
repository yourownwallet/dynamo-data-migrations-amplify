...
describe('up', () => {
    ...
    beforeEach(() => {
        ...
        migrationsDirLoadMigration = jest
            .spyOn(migrationsDir, 'loadFilesToBeMigrated')
        ...
        migrationsDbAddMigrationToMigrationsLogDb = jest
            .spyOn(migrationsDb, 'addMigrationToMigrationsLogDb')
            .mockReturnValue(Promise.resolve({ $metadata: {} }));
        migrationsDbConfigureMigrationsLogDbSchema = jest
            .spyOn(migrationsDb, 'configureMigrationsLogDbSchema')
    ...
    it('should populate the migrationsLogDb with info about the upgraded migrations', async () => {
        ...
        expect(migrationsDbAddMigrationToMigrationsLogDb).nthCalledWith(
            1,
            {
    ...
    it('should yield an error + items already migrated when unable to update the migrationsLogDb', async () => {
        migrationsDbAddMigrationToMigrationsLogDb
