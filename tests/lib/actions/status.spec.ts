...
describe('status', () => {
    ...
    beforeEach(() => {
        migrationsDirGetFileNames = jest
            .spyOn(migrationsDir, 'getFileNamesInMigrationFolder')
            .mockReturnValue([
    ...
    it('should yield an error when the migrations directory does not exist', async () => {
        ...
        await expect(status()).rejects.toThrow(
            'Please ensure migrations directory as specified in config.json is present',
        );
    });

    it('should yield errors that occurred when getting the list of files in the migrations directory', async () => {
        migrationsDirGetFileNames.mockImplementation(() => {
    ...
    it('should yield errors that occurred when fetching the migrationsLogDb collection', async () => {
        migrationsDbGetAllMigrations.mockImplementation(() => {
